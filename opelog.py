...
l = 2**n
v = [[i>>j&1 for j in range(n-1,-1,-1)] for i in range(l-1, -1, -1)]    ## una version corta para hacer una combinatoria de verdaderos y falsos empezando
...
while (option == "y")|(option == "s"):
    ...
    for f in range(len(v[0])):
        print(f)
        if all(v[n][f]==i for n,i in enumerate(lista)):             ## Comparamos nuestra nueva lista con los valores pasados y vemos si se parecen.
            print("esta expresion es igual a la lista "+str(f+1))

    for n,k in enumerate(v):
        k.append(lista[n])                          ## Finalmente almacenamos la lista en nuestra primera lista para hacer mas trabajos con ella
